...
public class AutoPickupCommand extends AbstractPlayerCommand {
    ...
    OptionalArg<String> actionArg = this.withOptionalArg("action", "toggle/settings/reload", ArgTypes.STRING);
    ...
    protected void execute(
        ...
        if (actionArg.provided(ctx)) {
            action = actionArg.get(ctx).toLowerCase();
        }

        switch (action) {
            case "toggle":
                handleToggle(playerRef);
                break;
            case "settings":
                handleSettings(store, ref, playerRef);
                break;
            case "reload":
                handleReload(playerRef);
                break;
            default:
                handleToggle(playerRef);
                break;
    ...
    private void handleToggle(PlayerRef playerRef) {
    ...
    private void handleSettings(Store<EntityStore> store, Ref<EntityStore> ref, PlayerRef playerRef) {
